        ...
        try:
            ...
            if response.status_code == 200:
                ...
                age_days = (datetime.now() - datetime.strptime(
                    data['created_date'][:10], '%Y-%m-%d')).days
...
def load_and_combine_datasets():
    ...
    for dataset in datasets:
        try:
            ...
            if dataset['col_mapping']:
                df = df.rename(columns=dataset['col_mapping'])
            ...
            df = df[dataset['required_cols']]  # Keep only required columns
...
def train_pipeline():
    ...
    X_train = hstack([X_train_subject, X_train_body, X_train_eng_scaled])
    X_test = hstack([X_test_subject, X_test_body, X_test_eng_scaled])
...
class PhishingDetector:
    def __init__(self, model_type='voting'):
        ...
        :param model_type: 'voting' or 'stacked'
        """
        self.model_type = model_type
        # if model_type == 'voting':
        self.model = joblib.load("phishing_model_voting.pkl")
        # else:
        #     self.model = joblib.load("phishing_model_stacked.pkl")
    ...
    def predict(self, email_text):
        ...
        # Create a DataFrame with the email text
        df = pd.DataFrame({
            'subject': [email_text['subject']],
            'body': [email_text['body']]
        })
        
        # Generate features with reputation system
        ...
        # Combine features
        X = hstack([subject_vec, body_vec, scaled_features])
        
        # Get probabilities and apply optimal threshold
        y_pred_proba = self.model.predict_proba(X)[0]
        y_pred = 1 if y_pred_proba[1] >= self.threshold else 0
        
        # Post-process prediction based on domain analysis
        ...
        # Rule 1: High reputation sender override
        if sender_reputation > 0.85:
            y_pred = 0
            y_pred_proba = np.array([0.95, 0.05])
            
        # Rule 2: Low reputation URL penalty
        ...
        probabilities = {
            "Legit": round(float(y_pred_proba[0]), 2),
            "Phishing": round(float(y_pred_proba[1]), 2)
        }
...
def main():
    ...
    predict_parser.add_argument('--model', type=str, choices=['voting', 'stacked'], 
                            default='voting', help='Model type to use for prediction')
